...
import { Keyboard, RefreshControl, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native'
...
const ProfitLoss = ({ navigation }: any) => {
  ...
  const { query, setQuery, filteredItems } = useSearchBar<any>(getData, {
    selector: (item) => String(item?.shift ?? ''),
    debounceMs: 200,
  });

  // Fetch Profit Loss Report data
  const getProfitLossReport = async (values: any) => {
    try {
      setLoading(true);
      const payload = {
      ...
      const response = await APIService.GetProfitLossReport(payload);
      if (response?.success && response?.data) {
        // Extract shift_data from response.data.shift_data or use response.data directly
        ...
        console.log('Profit Loss report data loaded:', shiftData.length, 'items');
        setData(shiftData);
      } else {
      ...
      console.error('Profit Loss report fetch failed', error);
      setData([]);
    } finally {
      setLoading(false);
    }
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <GradientBackground colors={["#fdf0d0", "#e0efea"]} locations={[0, 30]}>
        <SafeAreaView style={style.safeAreaContainer}>
          <ScreenHeader title={"ProfitLoss Report"} navigation={navigation} hideBackButton={true} showDrawerButton={true} >
              ...
              <BottomSheetScrollView style={{ paddingHorizontal: scale(10), backgroundColor: COLORS.BGFILESCOLOR, flex: 1 }}>
                ...
                <Formik
                  ...
                  onSubmit={(values, { resetForm }) => {
                    ...
                    getProfitLossReport(formValues);
                    handleClosePress();
                    // resetForm();
                  }}
                >
