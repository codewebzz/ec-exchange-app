...
export const Transaction = ({ navigation }: any) => {
  ...
  // Handle Edit Transaction (fetch details then navigate with full data)
  const handleEditTransaction = async (item: any) => {
    try {
      setLoadingDetails(true);
      const response = await APIService.GetTransactionData({}, item.id);
      let detailedData: any[] = [];
      if (response && response.success && response.data && Array.isArray(response.data.transactions)) {
        detailedData = response.data.transactions.map((t: any, idx: number) => ({
          id: t.id ?? idx + 1,
  ...
  // Function to fetch transactions based on active/deleted status
  ...
  // Fetch transactions on component mount (default to active transactions)
  useEffect(() => {
    // Set default value for activeDeleteShift if not set
    if (!activeDeleteShift) {
